...
const exactFeastDashHtml = `<!DOCTYPE html>
...
<body class="bg-[#0c0a09] text-stone-100 font-sans min-h-screen flex flex-col selection:bg-orange-500 selection:text-black">
  ...
  <script>
    ...
    window.filterCuisine = filterCategory;
...
async function applyExactCode() {
  await mongoose.connect(process.env.MONGODB_URL || process.env.MONGODB_URI);
  
  // Update site 6a8ffec04a7fe1e3d97ecff5
  const site = await Website.findById("6a8ffec04a7fe1e3d97ecff5");
  if (site) {
    site.latestCode = exactFeastDashHtml;
    await site.save();
    console.log("Updated site 6a8ffec04a7fe1e3d97ecff5 with EXACT FeastDash 72-dish code! ✅");
  }
